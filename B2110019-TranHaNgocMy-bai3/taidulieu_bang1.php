...
// Check connection
    ...
    $sql = "SELECT * FROM student a, major b WHERE a.id_major=b.id_major";
...
<table
    border=1><tr><th>ID</th><th>Hoten</th><th>email</th><th>ngaysinh</th><th>Ma Nganh</th><th>Ten Nganh</th><th colspan="2">Hanh dong</th></tr>
    <?php
// output data of each row
    foreach ($result_all as $row) {
        $date = date_create($row['Birthday']);
        echo "<tr><td>" . $row["id"]. "</td><td>" . $row["fullname"].
        "</td><td>" . $row["email"]. "</td><td>".$date ->format('d-m-Y')."</td><td>"
        .$row["id_major"]."</td><td>".$row["name_major"]."</td><td>";
    ?>
